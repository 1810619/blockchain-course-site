...
from flask import Flask,request    #从Flask框架导入Flask类（用于创建Flask应用）和request对象（用于处理HTTP请求）
import flask_socketio    #导入flask_socketio模块，它提供了对Socket.IO协议的支持，允许在Flask应用中使用WebSocket
app = Flask(__name__)    #创建一个Flask应用实例，__name__作为参数，赋值给app变量

# 定义变量名为my_socketio防止重名
# 创建一个Socket.IO服务器实例，并将其绑定到Flask应用实例app上。
# cors_allowed_origins='*' 配置内容表示允许所有来源的跨域请求，这在跨域环境下（例如浏览器与服务器处于不同的域）是必须的
my_socketio = flask_socketio.SocketIO(app, cors_allowed_origins='*')

# 使用装饰器创建以"message"为标识的socket接口
